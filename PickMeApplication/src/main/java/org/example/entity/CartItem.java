...
@Table(name = "cart_items", indexes = {
    ...
    @Index(name = "idx_cartitem_menu_item_id", columnList = "menu_item_id"),
...
public class CartItem {
    ...
    // Pricing (snapshot tại thời điểm add vào cart)
    @NotNull(message = "Unit price is required")
    @DecimalMin(value = "0.0", inclusive = false, message = "Unit price must be greater than 0")
    @Column(name = "unit_price", precision = 10, scale = 2, nullable = false)
    private BigDecimal unitPrice; // Giá tại thời điểm add vào cart
    ...
    // Snapshot của MenuItem info (để tránh thay đổi khi MenuItem bị sửa)
    ...
    public CartItem(Cart cart, MenuItem menuItem, Integer quantity, String specialInstructions) {
        this();
        this.cart = cart;
    ...
    private BigDecimal calculateTotalPrice() {
        BigDecimal addOnsTotal = addOns.stream()
            .map(CartItemAddOn::getTotalPrice)
    ...
    public void addAddOn(String name, String description, BigDecimal price, Integer quantity) {
        // Check if add-on already exists, then update quantity
        CartItemAddOn existingAddOn = addOns.stream()
        ...
        if (existingAddOn != null) {
            ...
            CartItemAddOn addOn = new CartItemAddOn(this, name, description, price);
    ...
    public void setAddOns(List<CartItemAddOn> addOns) {
        this.addOns = addOns;
    }
